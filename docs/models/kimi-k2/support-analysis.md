...
- H20 已做非 server 验证：dry-run rsync 后同步 tooling 文件，`PEGAINFER_TRITON_PYTHON=$PEGAINFER_DIR/.venv-kimi/bin/python` 下 release check 通过，`kimi_kernel_report trace --batch-size 4 --kv-len 1024` 生成同样 `1765` call 的 JSON。CUDA event provider 已在 H20 跑过 runtime bs1/kv2 和 static bs4/kv1024 model report；CUPTI 本阶段不接入。
...
- **Read**:
  ...
  - `$MODEL_DIR/config.json` — 文本核心是 `DeepseekV3ForCausalLM` 风格 `kimi_k2`，外层 `KimiK25ForConditionalGeneration` 的 vision/projector 本阶段忽略。
  - `$MODEL_DIR/modeling_deepseek.py` — 确认 MLA attention、YARN RoPE、`noaux_tc` MoE gate、shared expert 和 routed expert 数据流。
  - `$MODEL_DIR/model.safetensors.index.json` — 确认 64 shard、约 595GB，总体是一层一个 shard；routed expert 权重是 `weight_packed/weight_scale/weight_shape`。
  - `$MODEL_DIR/README.md` 与 `$MODEL_DIR/docs/deploy_guidance.md` — 记录官方 API 示例、vLLM/SGLang TP8 serve 命令、thinking/instant/preserve-thinking 参数。
  - `$MODEL_DIR/chat_template.jinja` 与 `$MODEL_DIR/tokenization_kimi.py` — 确认 text-only prompt 渲染、thinking 开关、tool declaration 和 tool call section 格式。
...
- 2026-05-21 rank expert loader 提供互斥的 CUTLASS probe package 与 Marlin/WNA16 package 路线；full-rank runtime state 不能同时常驻两套 routed expert package。Marlin package 最终常驻 fused W13 + W2，包含 uint4b8 no-actorder packed weight 与 group-major+perm64 BF16 scale，后续 compute ABI 不再依赖 checkpoint raw tensors。H20 已用真实 `$MODEL_DIR` 验证 rank0 60 层 fused W13 + W2 package load、Marlin weight no-act、scale perm64、route alignment 和 typed GPU view。
- 2026-05-21 Marlin/WNA16 compute wrapper 必须按 H20 vLLM `0.19.0` ABI 接，不按更新的 yingshan vLLM header：当前 fixture op 没有 `is_ep` 参数。H20 单层 synthetic W13 + SwiGLU + W2 + topk reduce 已对 vLLM reference 0-diff；真实 `$MODEL_DIR` rank0 layer1 fixture gate 同样在 W13、SwiGLU 后 W2、top-k sum 三段 0-diff。
...
- 2026-05-21 H20 server 端到端 gate：`pegainfer-server` 用真实 `$MODEL_DIR` 启动成功，engine load `131754ms`；raw `"Hello"` 请求返回 token id `950`；vLLM fixture 27-token prompt 返回 token id `1008`，与既有 vLLM greedy 对齐。`max_tokens=2` 当前仍只返回 1 token，`logprobs=5` 当前因 Kimi scheduler 不返回 logprob payload 而 500。
...
- 2026-05-21 nsys tail profile：H20 trace 产物在 `$RESULT_ROOT/pegainfer-kimi-nsys/kimi_bs4_decode.{nsys-rep,sqlite}`，summary 在 `$RESULT_ROOT/pegainfer-kimi-nsys/tail-summary.md`。本轮明确把 `std/p95/p99/max` 纳入性能 gate：BF16 all-reduce `p50=74.7us` 但 `p99=780us`、`max=2.98ms`；F32 all-reduce `p50=64.8us` 但 `p99=385us`、`max=886us`；Marlin WNA16 `p50=14.3us` 但 `p99=154us`、`max=187us`；`cuStreamSynchronize` `p50=28.3us` 但 `p99/max=9.87ms`。这说明 tail/rank skew/API drain 比单看 p50 更关键。
...
- 结果：
  - 拉取完成：`$MODEL_DIR`
  - `find $MODEL_DIR -type f \( -name '*.safetensors' -o -name '*.bin' -o -name '*.pt' -o -name '*.pth' -o -name '*.ckpt' -o -name '*.gguf' \)` 无输出。
  - 目录大小约 `26M`。
...
- 已从 `$RESULT_ROOT/pegainfer-kimi-k2-headers` 把 header/API 草案迁入工作区 `pegainfer-kimi-k2/`。
...
- 本地 `$MODEL_DIR/model.safetensors.index.json` 验证结果：
  - text tensor：`208215`
...
- H20 权重路径：`$MODEL_DIR`
- 已知节点提示符记录：`root@<H20_NODE>:$MODEL_DIR#`
- `Kimi-K2.5` 与 `Kimi-K2.6` 架构相同，K2.6 是继续训练后的版本；算子、shape、TP8/EP8 规划可共用。
- 本地非权重参考路径仍是 `$MODEL_DIR`。
- 不下载模型权重；远端优先检查 `$MODEL_ROOT`。

### H20 vLLM reference 环境

- K2.5 reference 首选完整 native venv：`$VLLM_DIR/.venv/bin/python`。
- 使用该 venv 时把 `$VLLM_DIR/.venv/bin` 放在 `PATH` 前面；FlashInfer JIT 需要同一环境里的 `ninja`。
- 不拼 `PYTHONPATH` overlay，不补 `_C_stable_libtorch` 符号链接，不混用另一个 vLLM checkout 的 Python 包和 SO。
- 已探测：
  - `$VLLM_DIR/.venv`：`vllm 0.19.0`、`torch 2.10.0+cu128`、registry 含 `KimiK25ForConditionalGeneration`，可直接跑 `$MODEL_DIR`。
  - `$VLLM_DIR_ALT/.venv`：有 vLLM dev 源码环境，但当前 registry 不含 `KimiK25ForConditionalGeneration`，不是 K2.5 fixture 首选。
- Python API reference 命令：

```bash
ssh <H20_NODE> 'cd $PEGAINFER_DIR
export PATH=$VLLM_DIR/.venv/bin:$PATH
export VLLM_WORKER_MULTIPROC_METHOD=spawn
export CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7
$VLLM_DIR/.venv/bin/python \
  pegainfer-kernels/tools/kimi_k2/vllm_logits_reference.py \
  --model-path $MODEL_DIR \
  --out-dir $FIXTURE_ROOT/kimi-k2/k25_parity_vllm \
...
- 2026-05-21 运行通过：
  - 日志：`$RESULT_ROOT/kimi_k25_vllm_fixture_top20_20260521_030017.log`
  - 输出：`$FIXTURE_ROOT/kimi-k2/k25_hello_vllm/{metadata.json,prompt.json,reference.safetensors}`
...
- 2026-05-21 多 prompt vLLM fixture 运行通过：
  - 日志：`$RESULT_ROOT/kimi_k25_vllm_parity_20260521_085822.log`
  - 输出：`$FIXTURE_ROOT/kimi-k2/k25_parity_vllm/{cases.json,hello,math_short,self_intro_zh,code_rust}`
...
- Python API 再遇到 vLLM 内部 API 兼容问题时，备用路线是同一 venv 的 `vllm serve --model $MODEL_DIR`，通过 OpenAI-compatible HTTP `/v1/completions` 请求 `logprobs=True` 拿 serving top-logprobs；仍不走 overlay。
- 2026-05-21 HTTP fixture 运行通过：
  - 启动环境：`$VLLM_DIR/.venv/bin/vllm serve $MODEL_DIR`，TP8，`--served-model-name kimi-k2.5`，`--trust-remote-code`，`--max-model-len 4096`，`--gpu-memory-utilization 0.85`，`--enforce-eager`。
  - 日志：`$RESULT_ROOT/kimi-k2-vllm-http/server_20260521_031017.log`
  - 输出：`$FIXTURE_ROOT/kimi-k2/k25_hello_vllm_http/{request.json,response.json,metadata.json,prompt.json}`
  ...
  - server 生成 fixture 后已停止，H20 node GPU 已释放。
...
```bash
ssh <H20_NODE> 'nvidia-smi'
ssh <H20_NODE> 'kubectl get pods -A -o wide --field-selector=status.phase!=Succeeded,status.phase!=Failed | awk '\''NR==1 || ($1 !~ /^(kube-system|monitor|istio-system|calico-system|tigera-operator)$/ && $8 == "<H20_NODE>")'\'''
```

- 如 `H20 node` 上有活跃 GPU 进程或非系统 pod，占用未释放前换节点验证。
...
1. 在远端工作目录下 clone 仓库，例如 `~/develop/<user>/pegainfer-kimi-k2-main`；如从 `main` 建基线，先 `git fetch origin main && git pull --ff-only origin main`。
...
rsync -avcn --relative \
  ...
  <H20_NODE>:~/develop/<user>/pegainfer-kimi-k2-main/
```

5. dry run 文件列表确认正确后，再执行同形态 rsync 去掉 `-n`。

6. 远端编译与验证在远端 checkout 内执行；H20 root 环境的 uv 在 `$USER_BIN_DIR/uv`，需要把 `$USER_BIN_DIR` 放进当前 session 的 PATH。Python/Triton 用 repo-local `.venv-kimi`，不改全局环境。
...
- 后续接算子 body 后，再用 `$MODEL_DIR` 做 text-only operator fixture 和 TP8/EP8 decode 验证。

### K2.5 rank0 权重加载 gate

- 测试入口：`h20_kimi_k25_rank0_sliced_payload_loads_typed_gpu_view`
- 模型路径硬编码为 `$MODEL_DIR`，不新增环境变量契约。
...
- 节点：`H20 node` / `H20 node`
- 基线：`~/develop/<user>/pegainfer-kimi-k2-main`，`origin/main@0cb1355`
- submodule：`flashinfer@779c24d1`，`cutlass@da5e086d`，`spdlog@c3aed4b6`
- 环境：`PEGAINFER_CUDA_SM=90a`，`PEGAINFER_TRITON_PYTHON=$PWD/.venv-kimi/bin/python`，`.venv-kimi` 由 `$USER_BIN_DIR/uv` 创建并安装 `triton==3.7.0`
...
- 结果：
  ...
  - `h20_kimi_k25_rank0_sliced_payload_loads_typed_gpu_view` 通过，耗时 `20.75s`；真实 `$MODEL_DIR` rank0 sliced payload 已加载到 GPU，并通过 typed GPU view 校验。
...
- 结果：
  - 2026-05-20：通过，耗时 `17.26s`。真实 `$MODEL_DIR` rank0 payload 已覆盖 sliced load、typed GPU view、expert-major package plan、layer1 raw buffer D2D package、CUTLASS sm90a reorder 和 typed `KimiInt4ExpertWeights` package。
  ...
  - 2026-05-21 Marlin package split gate：H20 `h20_kimi_k25_rank0_marlin_expert_package_loads` 通过，真实 `$MODEL_DIR` rank0 60 层 MoE 可打成 Marlin-only package，未再触发双 package OOM；`h20_kimi_k25_rank0_sliced_payload_loads_typed_gpu_view` 同步通过，CUTLASS probe package 路线未回退。
...
- 结果：
  ...
  - 测试结束后 H20 node GPU compute app 为空。
...
- 结果：
  ...
  - 输出：`$FIXTURE_ROOT/kimi-k2/pegainfer_k25_smoke_logits/{candidate.safetensors,metadata.json}`
  - candidate 文件大小 `655448` bytes，`logits_f32` shape `[163840]`。
  - prompt ids 来自 `$FIXTURE_ROOT/kimi-k2/k25_hello_vllm/prompt.json`，最后一个 token 是 `163606`。
  - smoke candidate argmax token id `154473`，logit `8.9375`；top8 ids `[154473, 159083, 160345, 161694, 149515, 159290, 161170, 149762]`。
  - 测试结束后 H20 node GPU compute app 为空。
  - H20 node 已扫现有 venv：`$VLLM_DIR/.venv`、`$VLLM_DIR_ALT/.venv`、`$PEGAINFER_DIR/.venv`、`$PEGAINFER_DIR/.venv` 等都没有 `accelerate`，不能直接作为 HF `device_map=auto` raw-logits env；不要用 PYTHONPATH overlay 补环境。本阶段不再继续补 HF env，先做 MLA + 全层 forward。
...
- 当前主线：
  ...
  - 用 `$FIXTURE_ROOT/kimi-k2/k25_parity_vllm` 多 prompt top-20 fixture 做 logits gate。
- 本地实现状态：
...
- 当前 H20 gate：
  ...
  - `h20_kimi_k25_dump_parity_prompt_candidates` 通过，用同一个 runtime load 依次 dump 4 个 prompt 的 full-vocab candidate，输出 `$FIXTURE_ROOT/kimi-k2/pegainfer_k25_parity_candidates`。
...
- **Outcome**: 已把 Kimi-K2.6 非权重文件拉到 `$MODEL_DIR`，确认没有权重大文件；文档范围改为 text-only operator bring-up，并拆出 P0/P1 TODO；已把官方推理示例、vLLM/SGLang serve 参数、chat template 契约、`pegainfer-kimi-k2` crate 初始化和 H20 验证规范纳入计划。
...
- **Lessons learned**:
  ...
  - H20 验证必须用 `$MODEL_DIR`，K2.5 与 K2.6 架构一致；远端 workspace 先从 `origin/main` 拉到最新，再用 rsync dry-run 补齐小范围源码，随后跑 sm90a 编译和真实 fixture。
